...
server.tool(
  "search_instana_docs",
  "Search IBM Instana Observability documentation. Returns matching topics with titles, snippets, and URLs.",
  {
    query: z.string().describe("Search query (e.g. 'monitoring', 'kubernetes', 'alerting')"),
...
server.tool(
  ...
  "Read a specific IBM Instana documentation page and return its content as Markdown. Use the 'href' from search results or TOC.",
  {
    href: z
      .string()
      .describe(
        "Document href path (e.g. 'SSE1JP5_1.0.315/...')"
      ),
  },
  async ({ href }) => {
    try {
...
server.tool(
  "get_instana_toc",
  "Get the table of contents for IBM Instana Observability documentation. Shows the full document structure with sections and topics.",
  {
    section: z
      .string()
      .optional()
      .describe("Optional: filter to a specific section by label (e.g. 'Installing', 'Monitoring', 'Alerting')"),
  },
  async ({ section }) => {
    try {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `# IBM Instana Observability - Table of Contents\n\n${formatToc(topics)}`,
          },
        ],
      };
    } catch (error) {
...
async function main() {
  ...
  console.error("Instana Docs MCP server running on stdio");
}
